...
static int check_edges(t_file *file, char **map)
{
	...
	// printf("y player %d\n", file->y);
	// printf("y player %d\n", y);
	// printf("len %d", (int)ft_strlen(map[file->y]));
	// printf("len %d", (int)ft_strlen(map[19]));
	// printf("linha trim end %s", result = ft_trim_end(map[14]));
	// printf("linha trim end len %d\n", (int)ft_strlen(result));
	// free(result);
	// printf("map y %s\n", map[y]);
    if (!map[y] || file->y == 0 || file->x == 0 || file->x == (int)(ft_strlen(map[file->y]) - 3)) // tem o \n por isso tiro 1
	{
